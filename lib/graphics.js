        ...
        if (isGraphicsController) {
          ...
          if (parts.length > 1 && parts[0].replace(/ +/g, '').toLowerCase().indexOf('region') !== -1 && parts[1].toLowerCase().indexOf('memory') !== -1) {
            ...
            if (memparts.length > 1) {
              let unit = /[0-9]{1,3}([a-zA-Z])/.exec(memparts[1])[1];
              let value = parseInt(memparts[1]);
              currentController.vram = unit.toLowerCase() === 'k' ? value / 1000 :
                unit.toLowerCase() === 'g' ? value * 1000 :
                unit.toLowerCase() === 't' ? value * 1000000 :
                value;
              console.log('currenController.vram', currentController.vram);
            }
          }
        }
      }
